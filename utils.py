...
class CycleLoss(nn.Module):
    def __init__(self, args):
        ...
        for key in ['lambda_' + n for n in names]:
            setattr(self, key, getattr(args, key))
        self.return_cycloss_per_sample = 'return_cycloss_per_sample' in args
        # print('return cycloss: ', self.return_cycloss_per_sample)
        if self.return_cycloss_per_sample:
    ...
    def loss_term_mean(self, dim):
        if not self.return_cycloss_per_sample:
            return
        for k in self.loss_names:
            _v = getattr(self, k)
            if torch.is_tensor(_v):
                setattr(self, k, torch.mean(_v, dim=dim))
        
    def forward(self, data, label, pred_s=None, pred_v=None, recon_s=None, recon_v=None):
        cal_loss = lambda x, y: (self.l1loss(x, y), self.l2loss(x, y))
        self.loss_g1v, self.loss_g2v, self.loss_g1s, self.loss_g2s = [0] * 4
        if pred_v is not None:
            self.loss_g1v, self.loss_g2v = cal_loss(pred_v, label) 
        if pred_s is not None:
            self.loss_g1s, self.loss_g2s = cal_loss(pred_s, data)

        self.loss_c1v, self.loss_c2v, self.loss_c1s , self.loss_c2s = [0] * 4
        if recon_v is not None:
            self.loss_c1v, self.loss_c2v = cal_loss(recon_v, label)
        if recon_s is not None:
            self.loss_c1s, self.loss_c2s = cal_loss(recon_s, data)

        self.loss_term_mean([1, 2, 3])
        
        loss = self.loss_g1v * self.lambda_g1v + self.loss_g2v * self.lambda_g2v + \
            self.loss_g1s * self.lambda_g1s + self.loss_g2s * self.lambda_g2s + \
            self.loss_c1v * self.lambda_c1v + self.loss_c2v * self.lambda_c2v + \
            self.loss_c1s * self.lambda_c1s + self.loss_c2s * self.lambda_c2s
        
        self.loss_term_mean(0)
        return loss, { k: getattr(self, k) for k in self.loss_names }
...
def init_distributed_mode(args):
    ...
    torch.distributed.init_process_group(backend=args.dist_backend, init_method=args.dist_url,
                                         world_size=args.world_size, rank=args.rank, timeout=datetime.timedelta(minutes=5))
